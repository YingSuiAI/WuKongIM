...
func TestDecodeCommandInspectionIncludesMessageEventWithoutPayload(t *testing.T) {
	...
	require.Equal(t, CommandInspection{
		...
		Payload: map[string]any{
			...
			"channel_type":  int64(2),
			"client_msg_no": "cmn-1",
			"run_id":        "run-1", "authorization_fence": "fence-1", "authority_sequence": uint64(1),
			"event_id":      "evt-1",
			"event_key":     "main",
			"event_type":    metadb.EventTypeDelta,
			"visibility":    metadb.VisibilityPublic,
...
func TestDecodeCommandInspectionIncludesMessageEventBatchWithoutPayloads(t *testing.T) {
	got, err := DecodeCommandInspection(EncodeAppendMessageEventsCommand([]metadb.MessageEventAppend{
		...
		},
		{
			...
			ChannelType: 2,
			ClientMsgNo: "cmn-1",
			RunID:       "run-2", AuthorizationFence: "fence-1", AuthoritySequence: 1,
			EventID:    "evt-finish",
			EventKey:   "main",
			EventType:  metadb.EventTypeFinish,
			Visibility: metadb.VisibilityPublic,
			...
			"events": []map[string]any{
				{
					...
					"event_key":     "main",
					"event_type":    metadb.EventTypeFinish,
					"visibility":    metadb.VisibilityPublic,
					"occurred_at":   int64(10),
					"updated_at":    int64(11),
				...
				},
				{
					...
					"channel_type":  int64(2),
					"client_msg_no": "cmn-1",
					"run_id":        "run-2", "authorization_fence": "fence-1", "authority_sequence": uint64(1),
					"event_id":      "evt-finish",
					"event_key":     "main",
					"event_type":    metadb.EventTypeFinish,
					"visibility":    metadb.VisibilityPublic,
