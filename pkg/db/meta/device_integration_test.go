...
func TestDeviceUpsertAndGetAreHashSlotScoped(t *testing.T) {
	...
	device := Device{UID: "u1", DeviceFlag: 1, DeviceID: "device-a", AppInstanceID: "app-a", Token: "left", DeviceLevel: 3}
	if err := left.UpsertDevice(context.Background(), device); err != nil {
		t.Fatalf("UpsertDevice(): %v", err)
	}
	got, ok, err := left.GetDevice(context.Background(), "u1", 1, device.DeviceID, device.AppInstanceID)
	if err != nil || !ok || got != device {
		t.Fatalf("left GetDevice() = (%+v, %v, %v), want %+v", got, ok, err, device)
	}
	if _, ok, err := right.GetDevice(context.Background(), "u1", 1, device.DeviceID, device.AppInstanceID); err != nil || ok {
		t.Fatalf("right GetDevice() = ok %v err %v, want missing", ok, err)
	}

	updated := Device{UID: "u1", DeviceFlag: 1, DeviceID: "device-a", AppInstanceID: "app-a", Token: "new", DeviceLevel: 4}
	if err := left.UpsertDevice(context.Background(), updated); err != nil {
		t.Fatalf("UpsertDevice(update): %v", err)
	}
	got, ok, err = left.GetDevice(context.Background(), "u1", 1, updated.DeviceID, updated.AppInstanceID)
...
func TestDeviceTableRuntimeDescriptor(t *testing.T) {
	...
	if got := deviceTable.Schema().Primary.Columns; len(got) != 4 {
		t.Fatalf("device primary columns = %#v, want uid, device flag, device id, and app instance id", got)
	}
}
