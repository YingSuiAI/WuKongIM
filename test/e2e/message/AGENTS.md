...
| `message_event_stream` | Prove service-only `/message/events:append` handles typed delta/snapshot/finish events, forwards from non-leader nodes, recovers from Slot-leader cache loss with snapshots, exposes public metrics, and survives restart through `/channel/messagesync` event summaries. | `GOWORK=off go test -tags=e2e ./test/e2e/message/message_event_stream -count=1 -timeout 2m` |
