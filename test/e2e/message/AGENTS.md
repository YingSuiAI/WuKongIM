...
| --- | --- | --- |
| `payload_correction` | Prove three-node create-only correction, claim/current reads, Slot leader change, restart and original SEND replay with 256 hash slots. | `GOWORK=off go test -tags=e2e ./test/e2e/message/payload_correction -count=1 -timeout=3m -p=1` |
...
| `conversation_directory_multi_node` | Prove Channel-Leader-grouped hydration, live committed-head reads, unavailable-Leader isolation with cursor progress, `/conversation/retry` recovery, UID membership reads through a four-node ingress outside the UID Slot replica set, and three-node group remove/reinvite convergence across service read-back, new-epoch history, and WKProto RECV. | `GOWORK=off go test -tags=e2e ./test/e2e/message/conversation_directory_multi_node -count=1 -timeout 3m -p=1` |
...
| `message_event_stream` | Prove canonical admitted anchors support service-only open/delta/snapshot/finish events, follower forwarding, Slot-leader snapshot recovery, public metrics, and restart replay with durable event summaries. | `GOWORK=off go test -tags=e2e ./test/e2e/message/message_event_stream -count=1 -timeout 2m` |
...
| `channel_failover` | Prove a static three-Controller-voter `cmd/wukongim` cluster preserves Channel quorum-acknowledged messages after one data node stops, fails over affected channel leaders, and creates new three-replica Channel metadata with a two-node healthy ISR while the stopped active member is unavailable. | `GOWORK=off go test -tags=e2e ./test/e2e/message/channel_failover -count=1 -timeout 3m -p=1` |
| `history_latest_page` | Prove latest-page sentinels and bounded older/forward pages preserve committed identities through all three HTTP ingresses with 256 hash slots. | `GOWORK=off go test -tags=e2e ./test/e2e/message/history_latest_page -count=1 -timeout=3m -p=1` |
